...
df = pd.read_csv('Final Assessment/Microsoft.csv', parse_dates=['Date'], index_col='Date')
print(df.head())
print(df.shape)
print(df.columns)
print(df.dtypes)
...
logistic_regressor = LogisticRegression(C=0.01, max_iter=1000, penalty='l2', solver='liblinear')
result = train_evaluate_classification_model('Logistic Regression', logistic_regressor)
print('Accuracy score: ', result['Accuracy score']) # Accuracy score:  0.532565130260521
...
dt_classifier = GridSearchCV(estimator=DecisionTreeClassifier(random_state=42), param_grid = {
                        ...
                        cv = TimeSeriesSplit(n_splits=5),
                        n_jobs=-1)
dt_classifier = DecisionTreeClassifier(max_depth=10, min_samples_leaf=2, min_samples_split=10,
                       random_state=42)
result = train_evaluate_classification_model('Decision Tree Classifier', dt_classifier)
print('Accuracy score: ', result['Accuracy score']) # Accuracy score:  0.5160320641282565
print('Confusion matrix: ', result['Confusion matrix'])
print('Classification report: ', result['Classification report'])
...
rf_classifier = GridSearchCV(estimator=RandomForestClassifier(random_state=42),    param_grid={
                        ...
                        cv = TimeSeriesSplit(n_splits=5),
                        n_jobs=-1)
rf_classifier = RandomForestClassifier(max_depth=10, min_samples_split=10, random_state=42)
result = train_evaluate_classification_model('Random Forest Classifier', rf_classifier)
print('Accuracy score: ', result['Accuracy score']) # Accuracy score:  0.498496993987976
print('Confusion matrix: ', result['Confusion matrix'])
print('Classification report: ', result['Classification report'])
...
print('R2 Score: ', result['R2 score']) # R2 Score:  -0.02612329499950583
...
dt_regressor = GridSearchCV(estimator=DecisionTreeRegressor(random_state=42),   param_grid={
    ...
    cv=TimeSeriesSplit(n_splits=5),
    n_jobs=-1)
dt_regressor = DecisionTreeRegressor(criterion='absolute_error', max_depth=5,
                      min_samples_split=10, random_state=42)
result = train_evaluate_regression_model('Decision Tree Regressor', dt_regressor)
print('R2 Score: ', result['R2 score']) # R2 Score:  0.0023621864201646137
print('Mean Absolute Error: ', result['Mean Absolute Error'])
print('Mean Squared Error: ', result['Mean Squared Error'])
print('Root Mean Squared Error: ', result['Root Mean Squared Error'])
plt.figure(figsize=(10,8))
...
rf_regressor = GridSearchCV(estimator=RandomForestRegressor(random_state=42), param_grid={
    ...
    cv=TimeSeriesSplit(n_splits=5),
    n_jobs=-1)
rf_regressor = RandomForestRegressor(max_depth=10, max_features='sqrt', min_samples_leaf=5,
                      random_state=42)
result = train_evaluate_regression_model('Random Forest Regressor', rf_regressor)
print('R2 Score: ', result['R2 score']) # R2 Score:  -0.025835681685979273
print('Mean Absolute Error: ', result['Mean Absolute Error'])
print('Mean Squared Error: ', result['Mean Squared Error'])
print('Root Mean Squared Error: ', result['Root Mean Squared Error'])
...
# R2 Score:  -126.26766500266753
# Mean Absolute Error:  0.18809649391141037
# Mean Squared Error:  0.04075293540712966
# Root Mean Squared Error:  0.2018735629227603
...
# Accuracy score:   0.4648241206030151
...
X_train = scaler.fit_transform(X_train)
X_test = scaler.transform(X_test)
logistic_regressor = LogisticRegression(C=0.1, max_iter=1000, penalty='l2', solver='liblinear')
...
# Logistic Regression Strategy vs Buy & Hold
...
# The Logistic Regression strategy increased the initial investment
# from 1 to approximately 3.82.
# This means that if we started with Rs. 100, it would become
# approximately Rs. 382.08 during the backtesting period,
# assuming no transaction costs or other expenses.
# Therefore, the strategy generated an overall return
# of approximately 282.08%.

# Buy & Hold Return:

# The Buy & Hold strategy increased the initial investment
# from 1 to approximately 4.33.
# This means that if we started with Rs. 100 and simply bought
# and held the stock, it would become approximately Rs. 432.94
# during the same period.
# Since the Logistic Regression strategy reached approximately 3.82
# while Buy & Hold reached approximately 4.33,
# the Buy & Hold strategy performed better in this backtest.

# Sharpe Ratio:

# Sharpe Ratio measures the return of the strategy compared
# with its overall risk.
# Our Sharpe Ratio is approximately 0.80.
# A positive Sharpe Ratio indicates that the strategy generated
# positive risk-adjusted performance during the backtesting period.
# A Sharpe Ratio of approximately 0.80 indicates that the strategy
# generated a reasonable return relative to the amount of risk taken.

# Sortino Ratio:

# Sortino Ratio is similar to Sharpe Ratio, but it focuses mainly
# on downside risk.
# Our Sortino Ratio is approximately 1.27.
# This indicates that the strategy generated positive returns
# relative to its downside risk during the backtesting period.
# Since the Sortino Ratio is higher than the Sharpe Ratio,
# the strategy's downside-risk-adjusted performance was
# relatively better than its overall risk-adjusted performance.

# Maximum Drawdown:

# Maximum Drawdown measures the largest fall in the strategy value
# from a previous peak to a later lowest point.
# Our Maximum Drawdown is approximately -35.02%.
# This means that at its worst point, the strategy lost approximately
# 35.02% from its previous highest value.
# Therefore, although the strategy generated a strong overall return,
# it also experienced a significant temporary decline and involved
...
# Our CAGR is approximately 18.44%.
# This means that the strategy grew at an average compounded
# annual rate of approximately 18.44% during the backtesting period.

# Overall Backtesting Conclusion:

# The Buy & Hold strategy performed better than the Logistic Regression
# strategy based on the final cumulative return.
# The Logistic Regression strategy achieved approximately 3.82 times
# the initial value, while Buy & Hold achieved approximately 4.33 times
# the initial value.
# The Logistic Regression strategy generated an overall return of
# approximately 282.08%, while Buy & Hold generated approximately
# 332.94%.
# The Sharpe Ratio of 0.80 and Sortino Ratio of 1.27 indicate
# positive risk-adjusted performance.
# The Maximum Drawdown of approximately -35.02% shows that the strategy
# experienced a significant temporary decline during the backtesting
# period and therefore involved considerable downside risk.
# The CAGR of approximately 18.44% indicates a strong annualized
# growth rate during the backtesting period.
# Overall, the Buy & Hold strategy showed better historical performance
# than the Logistic Regression strategy in this backtest based on
# final cumulative return.
# However, the Logistic Regression strategy also generated a strong
# positive return and demonstrated positive risk-adjusted performance.
# These backtesting results are based on historical data and do not
