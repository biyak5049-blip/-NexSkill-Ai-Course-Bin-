...
print('Accuracy score: ', result['Accuracy score']) # Accuracy score:  0.4485294117647059
...
dt_classifier = GridSearchCV(estimator=DecisionTreeClassifier(random_state=42), param_grid = {
                        ...
                        cv = TimeSeriesSplit(n_splits=5), 
                        n_jobs=-1)
dt_classifier = DecisionTreeClassifier(criterion='entropy', max_depth=15, min_samples_leaf=5, random_state=42)
result = train_evaluate_classification_model('Decision Tree Classifier', dt_classifier)
print('Accuracy score: ', result['Accuracy score']) # Accuracy score:  0.43014705882352944
print('Confusion matrix: ', result['Confusion matrix'])
print('Classification report: ', result['Classification report'])
...
rf_classifier = GridSearchCV(estimator=RandomForestClassifier(random_state=42),    param_grid={
                        ...
                        cv = TimeSeriesSplit(n_splits=5), 
                        n_jobs=-1)
rf_classifier = RandomForestClassifier(max_depth=5, min_samples_leaf=5, random_state=42)
result = train_evaluate_classification_model('Random Forest Classifier', rf_classifier)
print('Accuracy score: ', result['Accuracy score']) # Accuracy score:  0.4742647058823529
print('Confusion matrix: ', result['Confusion matrix'])
print('Classification report: ', result['Classification report'])
...
for i in range(len(X_columns)):
    item = {
        X_columns[i]: rf_classifier.feature_importances_[i]
    }
...
print('R2 Score: ', result['R2 score']) # R2 Score:  0.0047170451597084995
...
dt_regressor = GridSearchCV(estimator=DecisionTreeRegressor(random_state=42),   param_grid={
    ...
    cv=TimeSeriesSplit(n_splits=5),
    n_jobs=-1)
dt_regressor = DecisionTreeRegressor(max_depth=5, min_samples_leaf=5, random_state=42)
result = train_evaluate_regression_model('Decision Tree Regressor', dt_regressor)
print('R2 Score: ', result['R2 score']) # R2 Score:  -0.1173311500508798
print('Mean Absolute Error: ', result['Mean Absolute Error'])
print('Mean Squared Error: ', result['Mean Squared Error'])
print('Root Mean Squared Error: ', result['Root Mean Squared Error'])
plt.figure(figsize=(10,8))
...
print('R2 Score: ', result['R2 score']) # R2 Score:  -0.17022676652497193
print('Mean Absolute Error: ', result['Mean Absolute Error'])
print('Mean Squared Error: ', result['Mean Squared Error'])
print('Root Mean Squared Error: ', result['Root Mean Squared Error'])
...
# Random Forest Strategy vs Buy & Hold
...
# The Random Forest strategy increased the initial investment from 1 to approximately 1.33.
# This means that if we started with Rs. 100, it would become approximately Rs. 133
# during the backtesting period, assuming no transaction costs or other expenses.
# Therefore, the strategy generated an overall positive return of approximately 32.9%.

# Buy & Hold Return:

# The Buy & Hold strategy increased the initial investment from 1 to approximately 1.55.
# This means that if we started with Rs. 100 and simply bought and held the stock,
# it would become approximately Rs. 155 during the same period.
# Since the Random Forest strategy reached 1.33 while Buy & Hold reached 1.55,
...
# Our Sharpe Ratio is approximately 1.06.
# A positive Sharpe Ratio indicates that the strategy generated positive risk-adjusted performance.
...
# Our Sortino Ratio is approximately 1.95.
# This indicates that the strategy generated a positive return compared with the downside risk taken.
...
# Our Maximum Drawdown is approximately -15.64%.
# This means that at its worst point, the strategy lost approximately 15.64%
# from its previous highest value.
# Therefore, although the strategy generated a positive return, it also
# experienced a temporary decline and involved some downside risk.

# CAGR:

# CAGR stands for Compound Annual Growth Rate.
# It shows the average yearly compounded growth of the strategy.
# Our CAGR is approximately 30.17%.
# This means that the strategy grew at an average compounded annual rate
# of approximately 30.17% during the backtesting period.

# Overall Backtesting Conclusion:

# The Buy & Hold strategy performed better than the Random Forest strategy
# based on the final cumulative return.
# The Random Forest strategy achieved approximately 1.33 times the initial value,
# while Buy & Hold achieved approximately 1.55 times the initial value.
# The positive Sharpe Ratio of 1.06 and Sortino Ratio of 1.95
# indicate positive risk-adjusted performance.
# The Maximum Drawdown of approximately -15.64% shows that
# the strategy experienced a relatively moderate temporary decline.
# Overall, the Buy & Hold strategy showed better historical performance
# than the Random Forest strategy in this backtest.
# However, the Random Forest strategy also generated a positive return
# with relatively controlled downside risk.
# These backtesting results are based on historical data and do not guarantee
